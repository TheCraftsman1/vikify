...
import com.vikify.app.ui.VikifyApp
...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            com.vikify.app.ui.theme.VikifyTheme {
        ...
        try {
            ...
            // Cast SDK not available (e.g. no Play Services)
        }
    }
    ...
    private fun handleSpotifyCallback(intent: android.content.Intent?) {
        intent?.data?.let { uri ->
            // Handle both old (spotify-callback) and new (spotify/callback) formats
            val isSpotifyCallback = uri.scheme == "vikify" && 
                (uri.host == "spotify-callback" || uri.host == "spotify" || uri.path?.contains("callback") == true)
            
            if (isSpotifyCallback) {
                // Let SpotifyAuthManager handle the callback (PKCE flow)
                if (com.vikify.app.features.spotify.SpotifyAuthManager.handleAuthCallback(this, uri)) {
                    android.util.Log.d("MainActivity", "Spotify auth callback handled successfully")
                }
                // Also extract code for legacy handling
