...
import com.vikify.app.core.constants.QUEUE_DEBUG
...
import com.vikify.app.core.extensions.move
...
class QueueBoard(
    ...
    fun addQueue(
        ...
        if (match != null) { // found an existing queue
            ...
            // UNLESS forced to
            val containsAll = match != null && mediaList.all { s -> match.queue.any { s?.id == it.id } } // if is subset
    ...
    fun removeSong(item: MultiQueueObject, index: Int): Boolean {
        ...
        // update current position only if move will affect it
        if (index < currentMediaItemIndex) {
            newQueuePos--
        } else if (index == currentMediaItemIndex) {
            // BUG FIX: Only increment if queue is not empty after removal
            val remainingSize = item.getSize() - 1
            if (remainingSize > 0) {
                newQueuePos = (newQueuePos + 1).coerceIn(0, remainingSize)
            } else {
                newQueuePos = 0
            }
        } else {
    ...
    fun setCurrQueue(item: MultiQueueObject?, shouldResume: Boolean = true, forceClean: Boolean = false): Int? {
        ...
        if (forceClean) {
            try {
                ...
                // player.player.stop() <-- REMOVED: Redundant and causes initial playback stutter
                // player.player.clearMediaItems() <-- REMOVED: setMediaItems replaces everything
    ...
    var queueEntity = java.util.concurrent.PriorityBlockingQueue<PriorityJob>()
    var queueSongMap = java.util.concurrent.PriorityBlockingQueue<PriorityJob>()
    var jobActive = Mutex()
    ...
    private suspend fun databaseDispatcher() {
        ...
        jobActive.withLock {
            while (queueEntity.isNotEmpty() || queueSongMap.isNotEmpty()) {
                // DEADLOCK FIX: Don't use runBlocking inside suspend function
                delay(5000L)
