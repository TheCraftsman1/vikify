package com.vikify.app.features.spotify
...
import java.net.URL
import com.vikify.app.data.repository.db.InternalDatabase
...
class SpotifyRepository(
    ...
    /**
     * Get user's playlists (with pagination to fetch ALL playlists)
     * OFFLINE SUPPORT: Caches to DB on success, loads from DB on failure
     */
    suspend fun getUserPlaylists(): List<SpotifyPlaylist> = withContext(Dispatchers.IO) {
        // Safe database access using InternalDatabase import
        ...
        try {
            if (isLoggedIn) {
                ...
                while (nextUrl != null) {
                    val json = apiGet(nextUrl)
                    if (json == null) break // API failure
                    ...
                    if (items != null) {
                        for (i in 0 until items.length()) {
                            ...
                            val count = item.optJSONObject("tracks")?.optInt("total", 0) ?: 0
                            ...
                            // Cache to DB
                            database.upsertSpotifyPlaylistCache(
                                com.vikify.app.data.repository.db.entities.entities.SpotifyPlaylistCacheEntity(
                                    spotifyId = id,
                                    name = name,
                                    imageUrl = imageUrl,
                                    trackCount = count,
        ...
        // 2. If API returned nothing (offline/error), load from Cache
        if (result.isEmpty()) {
            Log.d(TAG, "Loading playlists from offline cache")
            val cached = database.getCachedSpotifyPlaylists().first()
            result.addAll(cached.map { 
                SpotifyPlaylist(
                    id = it.spotifyId,
                    name = it.name,
        ...
        Log.d(TAG, "Total playlists fetched: ${result.size} (Source: ${if (isLoggedIn && result.isNotEmpty()) "API" else "Cache"})")
        result
    }
    ...
    /**
     * Get tracks for a playlist (with pagination to fetch ALL tracks)
     * OFFLINE SUPPORT: Caches tracks to DB, loads from DB if offline.
     */
    suspend fun getPlaylistTracks(playlistId: String): List<SpotifyTrack> = withContext(Dispatchers.IO) {
        ...
        var isOnlineFetchSuccess = false
        ...
        try {
            if (isLoggedIn) {
                ...
                // If starting fresh fetch, we'll clear old mapping eventually, 
                // but let's do it transactionally or just overwrite. 
                // For now, we'll collect all valid tracks then update DB.
                val tracksToCache = mutableListOf<SpotifyTrack>()
                ...
                while (nextUrl != null) {
                    ...
                    if (items != null) {
                        for (i in 0 until items.length()) {
                            ...
                            if (trackParams != null && !trackParams.isNull("id")) {
                                 ...
                                 val trackId = trackParams.getString("id")
                                 val trackName = trackParams.getString("name")
                                 val imageUrl = trackParams.optJSONObject("album")?.optJSONArray("images")?.optJSONObject(0)?.optString("url")

                                val spTrack = SpotifyTrack(
                                    id = trackId,
                                    title = trackName,
                                    artist = artistName,
                                    imageUrl = imageUrl,
                ...
                if (result.isNotEmpty()) {
                    isOnlineFetchSuccess = true
                    // Update cache in background
                    try {
                        // 1. Clear old mapping
                        database.deleteSpotifyPlaylistTrackMaps(playlistId)
                        
                        // 2. Insert new mappings and songs
                        tracksToCache.forEachIndexed { index, track ->
                            // Ensure song exists (create basic entity if needed)
                            // We construct a SongEntity with special UNRESOLVED_ ID prefix if we don't know the Youtube ID yet.
                            // BUT, we should check if we already have it resolved.
                            
                            val existing = database.getBySpotifyId(track.id)
                            val songId = existing?.id ?: "UNRESOLVED_${track.id}"
                            
                            // Upsert song entity with Spotify metadata
                            database.insert(
                                SongEntity(
                                    id = songId,
                                    title = track.title,
                            ...
                            // Map to playlist
                            database.insertSpotifyPlaylistTrackMap(
                                com.vikify.app.data.repository.db.entities.entities.SpotifyPlaylistTrackMap(
        ...
        // 2. Fallback to cache if API failed or empty
        if (result.isEmpty()) {
            Log.d(TAG, "Loading playlist tracks from offline cache for $playlistId")
            val cachedSongs = database.getCachedSpotifyPlaylistSongs(playlistId).first()
            if (cachedSongs.isNotEmpty()) {
                result.addAll(cachedSongs.map { song ->
                    SpotifyTrack(
                        id = song.spotifyId ?: song.id.removePrefix("UNRESOLVED_"),
                        title = song.spotifyTitle ?: song.title,
                        artist = song.spotifyArtist ?: "Unknown", // We might lose artist if not in simple fields, but Spotify fields usually have it
                        imageUrl = song.spotifyArtworkUrl ?: song.thumbnailUrl,
                        duration = (song.duration * 1000).toLong(),
                        
                        // IMPORTANT: Pass the resolved YouTube ID if we have it!
                        // This allows instant playback for offline-resolved songs
                        youtubeId = if (!song.id.startsWith("UNRESOLVED_")) song.id else null
                    )
                })
            }
        }
        
        Log.d(TAG, "Total tracks fetched for playlist $playlistId: ${result.size} (Online: $isOnlineFetchSuccess)")
        result
    }
     ...
     */
    suspend fun loadPlaylistAndSync(playlistId: String, database: com.vikify.app.data.repository.db.MusicDatabase): List<SpotifyTrack> = withContext(Dispatchers.IO) {
        val tracks = getPlaylistTracks(playlistId)
    ...
    suspend fun importPlaylistSkeleton(
        playlistId: String, 
        database: com.vikify.app.data.repository.db.MusicDatabase
