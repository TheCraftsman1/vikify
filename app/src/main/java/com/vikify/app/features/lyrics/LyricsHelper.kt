...
import com.vikify.app.core.constants.LyricSourcePrefKey
import com.vikify.app.core.constants.LyricTrimKey
import com.vikify.app.core.constants.MultilineLrcKey
...
import com.vikify.app.data.repository.db.entities.entities.LyricsEntity.Companion.LYRICS_NOT_FOUND
...
class LyricsHelper @Inject constructor(
    ...
    // PROVIDER ORDER: LrcLib first (reliable synced lyrics database)
    // YouTube transcript often fails for music, only works for videos with captions
    private val lyricsProviders = listOf(
        LrcLibLyricsProvider,            // BEST: Reliable synced lyrics database (lrclib.net)
        KuGouLyricsProvider,             // Synced lyrics (Chinese/Asian songs)
        YouTubeSubtitleLyricsProvider,   // Synced video captions (when available)
        YouTubeLyricsProvider            // Plain text fallback (last resort)
    )
