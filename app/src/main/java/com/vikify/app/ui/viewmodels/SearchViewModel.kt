...
class SearchViewModel @Inject constructor(
    ...
    fun toggleSearchSource() {
        _searchSource.value = if (_searchSource.value == SearchSource.YOUTUBE) SearchSource.SPOTIFY else SearchSource.YOUTUBE
    }
    ...
    // Search result cache (LRU-style)
    private val resultCache = LinkedHashMap<String, List<UnifiedSearchResult>>(
                ...
                when {
                    ...
                    query.length < MIN_QUERY_LENGTH -> {
                        ...
                        if (cached != null) {
                            ...
                            _searchState.value = SearchState.Success(cached)
                            _unifiedResults.value = filterResults(cached, filter)
                        } else {
                            _searchState.value = SearchState.Loading
                            performSearch(query, source, filter)
                        }
                        // Also fetch suggestions for next keystroke
                        fetchYouTubeSuggestions(query)
                    }
                }
            }
    ...
    /**
     * Fetch YouTube search suggestions for autocomplete
     */
    private suspend fun fetchYouTubeSuggestions(query: String) {
        ...
        try {
            ...
            result.onSuccess { suggestions ->
                _searchSuggestions.value = suggestions.queries.take(6)
            }
        } catch (e: Exception) {
            // Silently fail - suggestions are non-critical
            Log.d(TAG, "Suggestions fetch failed: ${e.message}")
        }
    }
        ...
        return withContext(Dispatchers.IO) {
            ...
            result.fold(
                onSuccess = { page ->
                    ...
                    val unified = ytItems.mapNotNull { it.toUnifiedResult() }
                    // Cache the result
                    val cacheKey = "${SearchSource.YOUTUBE}:$query"
                    synchronized(resultCache) {
                        resultCache[cacheKey] = unified
