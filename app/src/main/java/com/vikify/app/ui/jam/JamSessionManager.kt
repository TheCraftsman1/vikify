...
class JamSessionManager @Inject constructor(
    ...
    // Track if Firebase is available - RECOVERY MODE: Only consistent failures trigger fallback
    // We don't want to permanently disable Firebase just because of one timeout
    private var isFirebaseUnreachable = false
    
    /**
     * Get current user ID, signing in anonymously if needed
     * Returns a local ID if Firebase auth fails, but allows retry later
     */
        ...
        return auth.currentUser?.uid ?: run {
             try {
                 ...
                 withTimeout(5000L) {
                     ...
                     isFirebaseUnreachable = false // Reset error flag on success
                     result.user?.uid ?: "local_${System.currentTimeMillis()}"
                 }
             } catch (e: Exception) {
                 Log.w(TAG, "Auth failed, using temporary local ID: ${e.message}")
                 // Do not set permanent fallback here, just return local ID for this operation
    ...
    suspend fun createSession(
        ...
        return try {
            ...
            // Try Firebase with timeout, with network availability check
            // Only attempt Firebase if Firebase was reachable before (avoid repeated offline errors)
            if (!isFirebaseUnreachable) {
                try {
                    ...
                    Log.w(TAG, "Firebase timeout, creating temporary local session")
                    // Do not set global fallback, just fail over for this session
                } catch (e: Exception) {
                    Log.w(TAG, "Firebase failed, creating temporary local session: ${e.message}")
                    // Do not set global fallback
                }
            }
            ...
            val localSession = session.copy(sessionId = "local_${System.currentTimeMillis()}")
    ...
    suspend fun joinSession(
        ...
        return try {
            ...
            if (code == "local" || code.startsWith("local_")) {
    ...
    suspend fun removeFromQueue(sessionId: String, itemId: String) {
        if (sessionId.startsWith("local_") || sessionId.startsWith("session_")) return
    ...
    // 
    // HEARTBEAT & ONLINE STATUS - Detect disconnected participants
    // 
    
    private var heartbeatJob: kotlinx.coroutines.Job? = null
    private var currentHeartbeatSessionId: String? = null
    ...
    fun startHeartbeat(sessionId: String) {
        ...
        heartbeatJob?.cancel()
        currentHeartbeatSessionId = sessionId
        ...
        heartbeatJob = CoroutineScope(Dispatchers.IO).launch {
            ...
            while (isActive) {
                try {
                    updateOnlineStatus(sessionId, userId, true)
                } catch (e: Exception) {
        ...
        Log.d(TAG, "Started heartbeat for session")
    }
    ...
    suspend fun stopHeartbeat() {
        ...
        val sessionId = currentHeartbeatSessionId ?: return
        val userId = auth.currentUser?.uid ?: return
        
        try {
            updateOnlineStatus(sessionId, userId, false)
        } catch (e: Exception) {
            Log.w(TAG, "Failed to mark offline")
        }
        currentHeartbeatSessionId = null
    }
    
    private suspend fun updateOnlineStatus(sessionId: String, participantId: String, isOnline: Boolean) {
        try {
            withTimeout(3000L) {
                val updates = mapOf(
                    "participants/$participantId/isOnline" to isOnline,
                    "participants/$participantId/lastSeen" to System.currentTimeMillis()
                )
                sessionsRef.child(sessionId).updateChildren(updates).await()
            }
        } catch (e: Exception) { }
        ...
        try {
            ...
            withTimeout(10000L) {
                ...
                snapshot.children.forEach { child ->
                    val expiresAt = child.child("expiresAt").getValue(Long::class.java) ?: 0
                    if (now > expiresAt) {
                        val sessionId = child.key ?: return@forEach
                        val code = child.child("sessionCode").getValue(String::class.java)
                        child.ref.removeValue().await()
                        code?.let { codesRef.child(it).removeValue().await() }
                        Log.d(TAG, "Cleaned up expired session: $sessionId")
                    }
                }
            }
        } catch (e: Exception) {
            Log.w(TAG, "Session cleanup failed")
        }
    }
}
